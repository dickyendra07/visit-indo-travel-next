...
export function ItineraryPreview() {
  return (
    <section className="bg-[#07110e] py-24 text-white md:py-32">
      <div className="mx-auto grid max-w-[1240px] gap-10 px-6 md:px-10 lg:grid-cols-[0.88fr_1.12fr] lg:items-start">
        <div className="lg:sticky lg:top-32">
          ...
          <p className="mt-6 max-w-xl text-[15px] leading-8 text-white/62 md:mt-7 md:text-[17px] md:leading-9">
        ...
        <div className="relative">
          ...
          <div className="mobile-scroll md:grid md:gap-5">
            {featuredItinerary.itineraryPreview.map((item, index) => (
              <article
                key={item.day}
                className="mobile-snap-card relative border border-white/10 bg-white/[0.055] p-5 backdrop-blur-md transition duration-500 hover:-translate-y-1 hover:border-[#d7aa51]/45 hover:bg-white/[0.075] md:ml-16 md:p-7"
              >
                ...
                <p className="mb-4 text-[10px] font-semibold uppercase tracking-[0.24em] text-[#d7aa51] md:text-[11px]">
                  {item.day}
                </p>
                <h3 className="font-display text-[30px] leading-none md:text-[34px]">
                  {item.title}
                </h3>
                <p className="mt-4 text-[14px] leading-7 text-white/62 md:mt-5 md:text-[15px] md:leading-8">
                  {item.description}
                </p>
              </article>
            ))}
          ...
          <p className="mt-4 text-center text-[11px] uppercase tracking-[0.2em] text-white/38 md:hidden">
            Swipe itinerary
          </p>
          ...
          <div className="mt-7 md:hidden">
            <Link
              href={`/journeys/${featuredItinerary.slug}`}
              className="inline-flex w-full justify-center rounded-full bg-white px-8 py-4 text-[11px] font-bold uppercase tracking-[0.18em] !text-[#07110e]"
            >
              Explore Full Journey
            </Link>
          </div>
